-- ================================================================
-- MJM — Transfer Plot Record: correction + import, in one go
--
-- Everything the office sheet needs, in the right order:
--   PART A  correct the existing batch 224 N18 397 record (N18-R -> N1)
--   PART B  import all 118 movements from the sheet
--
-- Run in the Supabase SQL Editor, top to bottom, in one paste.
-- Nothing is written until PART A section 2 and PART B section 4; every
-- other section only reads and reports. Safe to re-run — already-recorded
-- movements are skipped, so nothing doubles up.
--
-- Read these three outputs before trusting the result:
--   PART B section 2  batch numbers with no match, or more than one match
--   PART B section 3a target plots missing from shared_plots
--   PART B section 5  per-batch: on the sheet vs now recorded
-- ================================================================


-- ################################################################
-- #  PART A — correct the existing N18 record
-- ################################################################
